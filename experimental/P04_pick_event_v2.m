...
%    - Adds a time-domain diagnostic figure with two stacked panels:
%         top    = pressure(t)   - currently a straight line (artificial)
%         bottom = deformation(t) for SC, ED, Full skin overlaid
%      Makes it obvious which signal is the placeholder vs real data.
%
%  INPUT
...
%% --- 5b. Time-domain diagnostic figure --------------------------------
%   This is the single fastest way to see whether the analysis is being
%   driven by real signals or by the placeholder. Top panel = pressure(t),
%   bottom = deformation(t) per region. With the artificial linspace ramp
%   the top panel is a perfect straight line; the bottom panel may dip
%   below the dashed zero line wherever segmentation noise > true motion.
fdiag = figure('Name','Time-domain diagnostic','Color','w', ...
    'NumberTitle','off','Units','normalized','Position',[0.36 0.55 0.62 0.40]);
t_idx = (0:N_resample-1)';

ax_p = subplot(2,1,1, 'Color','w','XColor','k','YColor','k','GridColor',[.3 .3 .3]);
plot(ax_p, t_idx, pressure101, 'b-', 'LineWidth', 2);
ylabel(ax_p,'Pressure (artificial 0..1)','Color','k');
title(ax_p, ...
    'Pressure(t) -- ARTIFICIAL straight line; replace pressure101 with real trace', ...
    'Color','k');
grid(ax_p,'on'); xlim(ax_p,[0 N_resample-1]);

ax_d = subplot(2,1,2, 'Color','w','XColor','k','YColor','k','GridColor',[.3 .3 .3]);
plot(ax_d, t_idx, sc_def101  , 'r-'                  , 'LineWidth', 1.5); hold(ax_d,'on');
plot(ax_d, t_idx, ed_def101  , 'Color',[0.85 0.65 0] , 'LineWidth', 1.5);
plot(ax_d, t_idx, full_def101, 'Color',[0 0.55 0]    , 'LineWidth', 1.5);
yline(ax_d, 0, 'k--', 'LineWidth', 1);
xlabel(ax_d,'Resample index   (0 = start frame, 100 = peak frame)','Color','k');
ylabel(ax_d,'Deformation (mm)','Color','k');
lg2 = legend(ax_d, {'SC','ED','Full skin','zero'}, 'Location','best');
set(lg2,'Color','w','TextColor','k','EdgeColor','k');
title(ax_d,'Deformation(t) per region   (curves dipping below zero = segmentation noise)','Color','k');
grid(ax_d,'on'); xlim(ax_d,[0 N_resample-1]);
...
if save_results
    ...
    saveas(fdiag, 'P04_result_timeseries.png');
